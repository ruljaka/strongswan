...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
	...
	private void setNextProfile(VpnProfile profile)
	{
		synchronized (this)
		{
			Log.d(TAG, "startConnection:: " + (profile != null? profile.getName() : "null"));
	...
	private Notification buildNotification(boolean publicVersion)
	{
		...
		if (!publicVersion)
		{
			if (add_action)
			{
				...
				serviceIntent.setAction(CharonVpnService.DISCONNECT_ACTION);
				PendingIntent pending = PendingIntent.getService(
	...
	{
		Log.d(TAG, "JNI :: getTrustedCertificates :: alias ->  " + this.mCurrentCertificateAlias);
	...
	{
		Log.d(TAG, "JNI :: getUserCertificate :: alias ->  " + mCurrentUserCertificateAlias);
